...
// 运势算法
static unsigned long hash_fortune(const char *str)
{
...
static bool handle_fortune_command(const char *user_id, const char *msg_text, char *reply_out, size_t max_len)
{
    if (strcmp(msg_text, "未卜先知") != 0 && strcmp(msg_text, "概率测算") != 0)
    {
        return false;
    }
    ...
    char seed_str[128];
    snprintf(seed_str, sizeof(seed_str), "%s_%s", user_id, date_str);

    unsigned long h = hash_fortune(seed_str);
    int score = h % 100;
    ...
    {
        fortune = "大吉";
        bad_luck_prob = score % 10;
    }
    else if (score >= 70)
    {
        fortune = "吉";
        bad_luck_prob = 10 + (score % 20);
    }
    else if (score >= 40)
    {
        fortune = "尚可";
        bad_luck_prob = 30 + (score % 20);
    }
    else if (score >= 20)
    {
        fortune = "小凶";
        bad_luck_prob = 50 + (score % 20);
    }
    else if (score >= 5)
    {
        fortune = "凶";
        bad_luck_prob = 70 + (score % 15);
    }
    else
    {
        fortune = "大凶";
        bad_luck_prob = 85 + (score % 15);
    }

    if (strcmp(msg_text, "未卜先知") == 0)
    {
        snprintf(reply_out, max_len, "你好，探索者。汝今日之运势乃：【%s】", fortune);
    }
    else
    {
        snprintf(reply_out, max_len, "你好，探索者。汝今日之倒霉率乃：【%d%%】", bad_luck_prob);
    }
    return true;
}
...
static int ws_send_frame(const ws_frame_t *f, const uint8_t *payload, size_t payload_len, int timeout_ms)
{
    uint8_t out[2048];
    size_t pos = 0;
    if (!pb_write_tag(out, sizeof(out), &pos, 1, 0) || !pb_write_varint(out, sizeof(out), &pos, f->seq_id))
        return -1;
    if (!pb_write_tag(out, sizeof(out), &pos, 2, 0) || !pb_write_varint(out, sizeof(out), &pos, f->log_id))
        return -1;
    if (!pb_write_tag(out, sizeof(out), &pos, 3, 0) || !pb_write_varint(out, sizeof(out), &pos, (uint32_t)f->service))
        return -1;
    if (!pb_write_tag(out, sizeof(out), &pos, 4, 0) || !pb_write_varint(out, sizeof(out), &pos, (uint32_t)f->method))
        return -1;
    ...
    for (size_t i = 0; i < f->header_count; i++)
    {
        uint8_t hb[256];
        size_t hlen = 0;
        if (!ws_encode_header(hb, sizeof(hb), &hlen, f->headers[i].key, f->headers[i].value))
            return -1;
...
static void handle_message_event(cJSON *event)
{
    ...
    // 拦截器：如果消息内容是特定指令（如 "fortune"），则直接回复，不进入后续的消息队列和大模型处理流程。
    char fortune_reply[256];
    if (handle_fortune_command(sender_id, cleaned, fortune_reply, sizeof(fortune_reply)))
    {
        feishu_send_message(route_id, fortune_reply);
        ESP_LOGI(TAG, "Intercepted fortune command from %s", sender_id);
        cJSON_Delete(content_obj);
        return; // 拦截后 return，切断后续入队和大模型调用！
    }
